...
draw_text_kl_scale(x,y-50*image_yscale,real_text,16,9999,image_alpha,image_blend,0,0,font0,image_xscale*1/3,image_yscale*1/3,image_angle)
	...
	if global.buying_components = 1 && instance_exists(obj_andience1)
	{
	var v_x = obj_camera.v_x/1280
	var xx = x-v_x*48
	var yy = player.y-v_x*200
	
	var t_color = merge_color(c_white,c_red,obj_andience1.cannot_buy_alpha)

	draw_text_kl_scale(x,yy,"구매할 재료를 선택해주세요",64,-1,1*image_alpha,t_color,-1,0,font0,v_x/3.5,v_x/3.5,0)
	...
	draw_sprite_ext(spr_icon,4,xx-220*v_x,yy+42*v_x,v_x*0.6,v_x*0.6,0,c_white,1*image_alpha)

	draw_text_kl_scale(xx-180*v_x,yy+30*v_x,"현재 최대 체력",64,-1,1*image_alpha,c_white,-1,-1,font0,v_x/3.5,v_x/3.5,0)
	draw_text_kl_scale(xx-175*v_x,yy+50*v_x,string(player.max_hp),64,-1,1*image_alpha,c_white,-1,-1,font0,v_x/4,v_x/4,0)
	draw_text_kl_scale(xx-175*v_x,yy+70*v_x,"현재 보유중인 골드 : "+string(global.gold)+"  Gold",64,-1,1*image_alpha,c_white,-1,-1,font0,v_x/4,v_x/4,0)
	...
	draw_text_kl_scale(xx-180*v_x,yy+30*v_x,"강화 후",64,-1,1*image_alpha,c_white,-1,-1,font0,v_x/3.5,v_x/3.5,0)
	draw_text_kl_scale(xx-175*v_x,yy+50*v_x,string(player.max_hp)+"+50",64,-1,1*image_alpha,c_white,-1,-1,font0,v_x/4,v_x/4,0)
	draw_text_kl_scale(xx-175*v_x,yy+70*v_x,"필요한 골드 : "+string(global.hp_upgrage_money)+"  Gold",64,-1,1*image_alpha,c_white,-1,-1,font0,v_x/4,v_x/4,0)
	}
	
	if (global.show_time = 3)
	{
	var v_x = obj_camera.v_x/1280
	var xx = x-v_x*48
	var yy = player.y-v_x*200
	var yours = "초록색"
		if global.your_page = 1
		{
		yours = "파란색"
		}
	draw_text_kl_scale(xx-180*v_x,yy+30*v_x,"당신이 고른 면 : "+string(yours),64,-1,1*image_alpha,c_white,-1,-1,font0,v_x/3.5,v_x/3.5,0)
	draw_sprite_ext(spr_coin,global.coin_flip_n,x,yy+v_x*100,global.coin_flip,1,0,c_white,image_alpha)
	}
}
